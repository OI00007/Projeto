...
// Validate period
function isValidPeriod(period: unknown): period is 'daily' | 'weekly' | 'monthly' | 'yearly' {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      ...
      return new Response(
        JSON.stringify({ error: 'Autenticação necessária', code: 'UNAUTHORIZED' }),
    ...
    if (authError || !user) {
      ...
      return new Response(
        JSON.stringify({ error: 'Autenticação inválida', code: 'INVALID_AUTH' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 401 }
      );
    }
    ...
    if (!allowed) {
      ...
      return new Response(
        JSON.stringify({ error: 'Limite de requisições excedido', code: 'RATE_LIMITED' }),
        { 
          headers: { 
            ...corsHeaders, 
    ...
    try {
      ...
      return new Response(
        JSON.stringify({ error: 'Formato de requisição inválido', code: 'INVALID_REQUEST' }),
    ...
    if (!isValidType(type)) {
      return new Response(
        JSON.stringify({ error: 'Tipo deve ser "summary" ou "forecast"', code: 'INVALID_TYPE' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 }
      );
    }
    ...
    if (type === 'summary') {
      ...
      if (queryError) {
        ...
        return new Response(
          JSON.stringify({ error: 'Erro ao buscar dados financeiros', code: 'QUERY_ERROR' }),
      ...
      const profitMargin = income > 0 ? Math.round((profit / income) * 10000) / 100 : 0;
      ...
      if (queryError) {
        console.error(`[${requestId}] Query error:`, queryError.message);
        return new Response(
          JSON.stringify({ error: 'Erro ao buscar dados para previsão', code: 'QUERY_ERROR' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 500 }
        );
      }
      ...
      const avgIncome = months.length > 0 
        ? months.reduce((sum, m) => sum + monthlyData[m].income, 0) / months.length 
        : 0;
      const avgExpenses = months.length > 0 
        ? months.reduce((sum, m) => sum + monthlyData[m].expenses, 0) / months.length 
        : 0;
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        forecast.push({
          ...
          predictedExpenses: sanitizeNumber(avgExpenses * (0.97 + Math.random() * 0.06)),
    ...
    console.log(`[${requestId}] Financial insights generated in ${elapsed}ms:`, { userId: user.id, type, period });
    ...
    return new Response(
      JSON.stringify({ 
        error: 'Não foi possível gerar os insights financeiros. Tente novamente.',
        code: 'INTERNAL_ERROR'
      }),
      { 
