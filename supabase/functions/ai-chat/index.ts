...
function validateMessage(msg: unknown): msg is { role: string; content: string } {
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${requestId}] Authenticated user:`, user.id, `(${remaining} requests remaining)`);
...
${sanitizedContext ? `Contexto atual da fazenda: ${sanitizedContext}` : ''}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[${requestId}] AI gateway error: ${response.status} (${elapsed}ms)`);
