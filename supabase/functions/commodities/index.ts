    ...
    const prices: CommodityPrice[] = [
      {
        name: 'Soja',
        symbol: 'SOJA',
        price: Math.round(138.50 * variation(0) * 100) / 100,
        currency: 'BRL',
        unit: 'saca 60kg',
      ...
      },
      {
        name: 'Milho',
        symbol: 'MILHO',
        price: Math.round(58.20 * variation(1) * 100) / 100,
        currency: 'BRL',
        unit: 'saca 60kg',
      ...
      },
      {
        name: 'Café Arábica',
        symbol: 'CAFE',
        price: Math.round(1420.00 * variation(2) * 100) / 100,
        currency: 'BRL',
        unit: 'saca 60kg',
      ...
      },
      {
        name: 'Açúcar Cristal',
        symbol: 'ACUCAR',
        price: Math.round(142.80 * variation(3) * 100) / 100,
        currency: 'BRL',
        unit: 'saca 50kg',
      ...
      },
      {
        name: 'Algodão',
        symbol: 'ALGODAO',
        price: Math.round(112.50 * variation(4) * 100) / 100,
        currency: 'BRL',
        unit: 'arroba 15kg',
      ...
      },
      {
        name: 'Trigo',
        symbol: 'TRIGO',
        price: Math.round(78.90 * variation(5) * 100) / 100,
        currency: 'BRL',
        unit: 'saca 60kg',
...
serve(async (req) => {
  ...
  try {
    ...
    if (cachedData && (Date.now() - cachedData.timestamp) < CACHE_TTL) {
      ...
      return new Response(
        JSON.stringify({ success: true, ...cachedData.data as Record<string, unknown>, cached: true }),
    ...
    const historical: Record<string, Array<{ date: string; price: number }>> = {};
    for (const commodity of prices) {
      const history = [];
      for (let i = 30; i >= 0; i--) {
        const pastDate = new Date();
        pastDate.setDate(pastDate.getDate() - i);
        const daySeed = Math.floor((pastDate.getTime() - new Date(pastDate.getFullYear(), 0, 0).getTime()) / 86400000);
        const idx = prices.indexOf(commodity);
        const dailyVar = Math.sin(daySeed * 0.1 + idx * 2.7) * 0.03;
        history.push({
          date: pastDate.toISOString().split('T')[0],
          price: Math.round(commodity.price / (1 + (Math.sin((daySeed + pastDate.getFullYear()) * 0.1 + idx * 2.7) * 0.03)) * (1 + dailyVar) * 100) / 100,
        });
      }
      historical[commodity.symbol] = history;
    }

    const responseData = { prices, historical, generatedAt: new Date().toISOString() };
