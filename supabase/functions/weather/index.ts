...
function isValidCoordinate(value: unknown, type: 'latitude' | 'longitude'): boolean {
...
serve(async (req) => {
  ...
  try {
    ...
    const { latitude, longitude, city } = requestData as Record<string, unknown>;
    ...
    if (city) {
      ...
      const cities: Record<string, { lat: number; lon: number; name: string }> = {
        ...
        "ribeirao preto": { lat: -21.1775, lon: -47.8103, name: "Ribeirão Preto" },
      ...
      if (cityCoords) {
        ...
        resolvedCity = sanitizedCity.charAt(0).toUpperCase() + sanitizedCity.slice(1);
      }
    }
    ...
    if (cached && Date.now() < cached.expiresAt) {
      ...
      return new Response(JSON.stringify(cached.data), {
        headers: { ...corsHeaders, "Content-Type": "application/json", "X-Cache": "HIT", "X-Response-Time": `${elapsed}ms` },
      });
    }
    ...
    // Store in cache
    cache.set(cacheKey, { data: weatherData, expiresAt: Date.now() + CACHE_TTL });

    const elapsed = Date.now() - startTime;
    console.log(`Weather fetched in ${elapsed}ms for ${resolvedCity}`);

    return new Response(JSON.stringify(weatherData), {
      headers: { ...corsHeaders, "Content-Type": "application/json", "X-Cache": "MISS", "X-Response-Time": `${elapsed}ms` },
    });
  } catch (error) {
    console.error("Weather error:", error instanceof Error ? error.message : "Unknown error");
    
    return new Response(
      JSON.stringify({ error: "Erro ao buscar dados meteorológicos. Tente novamente." }),
