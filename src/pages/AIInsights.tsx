...
const AIInsights = () => {
    ...
    async (text?: string) => {
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantId
              ? { ...m, content: `⚠️ Erro ao processar. Tente novamente.` }
              : m,
          ),
        );
      } finally {
  ...
  const insights = [
    ...
    },
    {
      ...
      description: humSensor
        ? humSensor.value < 35
          ? "🚿 Irrigação urgente recomendada!"
          : humSensor.value < 55
            ? "💧 Irrigação leve nos próximos dias."
    ...
    },
    {
      ...
      bg: "bg-warning/10",
      title: tempSensor
        ? `Temperatura: ${tempSensor.value}°C`
        : "Sem sensor de temperatura",
      description: tempSensor
        ? tempSensor.value > 35
          ? "⚠️ Temperatura elevada — estresse hídrico possível."
          : tempSensor.value < 10
            ? "❄️ Risco de geada — proteja as culturas."
  ...
  return (
    <DashboardLayout
      title="IA & Insights"
      subtitle="Análise inteligente e chat com Argom AI"
    >
                  ...
                  >
                    <CardContent className="pt-5 pb-4">
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-9 h-9 rounded-xl ${ins.bg} flex items-center justify-center flex-shrink-0`}
                        >
                ...
                {[
                  {
                    ...
                    text:
                      criticalSensors.length > 0
                        ? `Verifique em campo: ${criticalSensors[0]?.name}`
                        : "Todos os sensores em ordem — faça ronda semanal",
                  },
                  {
                    icon: Droplets,
                    text:
                      humSensor && humSensor.value < 50
                        ? "Irrigação recomendada nas próximas horas"
                        : "Umidade do solo adequada",
                  },
                  {
                    icon: Target,
                    text: `${getCurrentHarvestYear()} — monitore preços das commodities`,
                  },
                  {
                    icon: TrendingUp,
                    text:
                      financial.profit > 0
                        ? "Resultado positivo — considere reinvestir em melhorias"
                        : "Revise as categorias de maior despesa",
                  },
                ].map((rec, i) => {
                  const Icon = rec.icon;
                  return (
            ...
            <Card className="flex flex-col" style={{ height: "600px" }}>
              ...
              <div className="px-4 pb-4 border-t pt-3 flex-shrink-0">
                ...
                <p className="text-xs text-muted-foreground mt-1.5">
                  O Argom AI usa os dados reais dos seus sensores e financeiro
                  para responder.
                </p>
              </div>
            </Card>
