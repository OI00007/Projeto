...
function setCache(city: string, data: WeatherApiData) {
  try {
    localStorage.setItem(
      `argom_weather_${city}`,
      JSON.stringify({ data, exp: Date.now() + CACHE_TTL }),
    );
  } catch {
    /**/
  }
...
async function geocode(
  ...
  const res = await fetch(url, {
    headers: {
      "User-Agent": "Argom-AgriPlatform/1.0",
...
async function fetchClimate(
  lat: number,
  lon: number,
): Promise<Omit<WeatherApiData, "location">> {
  ...
  const hourly = r.hourly.time
    .map((t: string, i: number) => ({
      ...
      weatherCode: r.hourly.weather_code[i],
    }))
    .filter((h: { time: string }) => h.time >= nowIso)
    .slice(0, 24);
  const daily = r.daily.time.map((date: string, i: number) => ({
    ...
    precipitationSum: r.daily.precipitation_sum[i] ?? 0,
...
export function useWeatherApi(city?: string): UseWeatherApiReturn {
  ...
  const [data, setData] = useState<WeatherApiData | null>(() =>
    getCache(resolved),
  );
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      if (!geo)
        throw new Error(
          `Cidade "${resolved}" não encontrada. Tente com o nome completo.`,
        );
      const climate = await fetchClimate(geo.lat, geo.lon);
      const full: WeatherApiData = {
        ...climate,
        location: {
          latitude: geo.lat,
          longitude: geo.lon,
