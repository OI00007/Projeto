    ...
    async (
      ...
      for (let attempt = 0; attempt <= (mergedOptions.retries ?? 0); attempt++) {
        try {
          ...
          if (signal.aborted) {
            setState({ data: null, error: 'Requisição cancelada', loading: false });
            return null;
          }

          // Don't retry on client errors
          if (lastError.message.includes('Sessão expirada') || 
      ...
      secureLog('error', 'API request failed', { url, error: lastError?.message });
      setState({ data: null, error: genericError, loading: false });
      return null;
    },
    [session?.access_token, mergedOptions.retries, mergedOptions.retryDelay, mergedOptions.timeout]
  );
 ...
 */
export function useEdgeFunction<T>(functionName: string, options: SecureApiOptions = {}) {
  const api = useSecureApi<T>(options);

  const invoke = useCallback(
    async (body: Record<string, unknown> = {}) => {
      const url = `https://cfcnuemvftiphexvjhvv.supabase.co/functions/v1/${functionName}`;
