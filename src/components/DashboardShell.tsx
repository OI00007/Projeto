...
const NAV_ITEMS = [
  ...
  { label: "IA & Insights",path: "/ai-insights" },
...
export function DashboardShell() {
  ...
  const suggestions = debouncedSearch.trim()
    ? NAV_ITEMS.filter(i => i.label.toLowerCase().includes(debouncedSearch.toLowerCase()))
    : [];

  function handleSelect(path: string) {
    setSearch(""); setShowSuggestions(false); navigate(path);
  }
      ...
      <SidebarProvider>
        <div className="min-h-screen flex w-full bg-gradient-to-br from-background via-background to-muted/10">
          ...
          <div className="flex-1 flex flex-col overflow-y-auto">
            ...
            {!isLoading && onboarding.isDemo && onboarding.step !== "welcome" && (
              <DemoBanner onSetupFarm={() => { goToStep("welcome"); setShowWizard(true); }} />
            )}
            ...
            <header className="h-16 border-b bg-background/90 backdrop-blur-2xl flex items-center px-4 sm:px-6 sticky top-0 z-40 shadow-sm">
              ...
              <div className="flex-1 flex items-center justify-between gap-4">
                ...
                <div className="flex-1 max-w-md hidden sm:block relative">
                  <div className="relative group">
                    ...
                    <Input
                      ...
                      onKeyDown={e => { if (e.key === "Escape") { setSearch(""); setShowSuggestions(false); } }}
                  ...
                  {showSuggestions && suggestions.length > 0 && (
                    <ul className="absolute top-full mt-1 left-0 w-full bg-background border rounded-xl shadow-xl z-[100] overflow-hidden">
                      {suggestions.map(item => (
                        <li key={item.path} onMouseDown={() => handleSelect(item.path)}
                ...
                <div className="flex items-center gap-2">
                  <Badge variant="secondary" className="hidden sm:flex gap-2 px-3 py-1.5 bg-success/10 text-success border-success/20 rounded-full text-xs">
